...
export class AttendanceRecord1741138161035 implements MigrationInterface {
  name = 'AttendanceRecord1741138161035';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE \`absence_justification\` (\`id\` varchar(36) NOT NULL, \`created_at\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, \`updated_at\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, \`deleted_at\` timestamp NULL, \`justification\` varchar(255) NOT NULL, \`studentAttendanceId\` varchar(36) NULL, UNIQUE INDEX \`REL_f02859c175d3a89cc585a2ef69\` (\`studentAttendanceId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DROP INDEX \`REL_f02859c175d3a89cc585a2ef69\` ON \`absence_justification\``,
    );
