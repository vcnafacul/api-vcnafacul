...
export class AttendanceRecord21741116269177 implements MigrationInterface {
  name = 'AttendanceRecord21741116269177';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE \`student_attendance\` (\`id\` varchar(36) NOT NULL, \`created_at\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, \`updated_at\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, \`deleted_at\` timestamp NULL, \`present\` tinyint(1) NOT NULL, \`studentCourseId\` varchar(36) NULL, \`attendanceRecordId\` varchar(36) NULL, \`justificationId\` varchar(36) NULL, UNIQUE INDEX \`IDX_2d5ea3fa66a07b9e95a86563e1\` (\`attendanceRecordId\`, \`studentCourseId\`), UNIQUE INDEX \`REL_8bcaa220df8f486625ac0ab2f0\` (\`justificationId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE \`student_attendance\` ADD CONSTRAINT \`FK_8bcaa220df8f486625ac0ab2f06\` FOREIGN KEY (\`justificationId\`) REFERENCES \`absence_justification\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`,
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE \`student_attendance\` DROP FOREIGN KEY \`FK_8bcaa220df8f486625ac0ab2f06\``,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `DROP INDEX \`REL_8bcaa220df8f486625ac0ab2f0\` ON \`student_attendance\``,
    );
    await queryRunner.query(
