...
export class ConviteColaborador1790270693123 implements MigrationInterface {
    ...
    /*
      ⚠️ Gerada pelo TypeORM, com um ajuste: o registro da coluna gerada no
    ...
    */
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`convites_colaborador\` (\`id\` varchar(36) NOT NULL, \`created_at\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, \`updated_at\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, \`deleted_at\` timestamp NULL, \`email\` varchar(255) NOT NULL, \`partner_prep_course_id\` varchar(255) NOT NULL, \`role_id\` varchar(255) NOT NULL, \`convidado_por\` varchar(255) NOT NULL, \`token_hash\` varchar(64) NOT NULL, \`status\` enum ('pendente', 'aceito', 'cancelado', 'expirado') NOT NULL DEFAULT 'pendente', \`expira_em\` timestamp NOT NULL, \`aceito_por\` varchar(255) NULL, \`chave_ativa\` varchar(300) AS (IF(\`status\` = 'pendente', CONCAT(\`email\`, ':', \`partner_prep_course_id\`), NULL)) STORED NULL, UNIQUE INDEX \`IDX_convite_colaborador_chave_ativa\` (\`chave_ativa\`), UNIQUE INDEX \`IDX_0148031d7d5ad1aafb6bd80022\` (\`token_hash\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
