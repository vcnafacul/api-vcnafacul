...
export class EssaySettings1774214837792 implements MigrationInterface {
  name = 'EssaySettings1774214837792';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE \`essay_settings\` (\`id\` varchar(50) NOT NULL DEFAULT 'default', \`ai_enabled\` tinyint NOT NULL DEFAULT 1, \`updated_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `ALTER TABLE \`roles\` ADD \`revisar_redacoes\` tinyint NOT NULL DEFAULT 0`,
    );
    await queryRunner.query(
      `ALTER TABLE \`roles\` ADD \`revisar_todas_redacoes\` tinyint NOT NULL DEFAULT 0`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`roles\` DROP COLUMN \`revisar_todas_redacoes\``,
    );
    await queryRunner.query(
